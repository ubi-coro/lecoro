...
python lecoro/scripts/control_robot.py teleoperate \
    --fps 30 \
    --robot-path lecoro/configs/robot/your_robot_config.yaml \
    --sim-config lecoro/configs/env/your_sim_config.yaml
```

- Record one episode in order to test replay:
```bash
python lecoro/scripts/control_sim_robot.py record \
    --robot-path lecoro/configs/robot/your_robot_config.yaml \
    --sim-config lecoro/configs/env/your_sim_config.yaml \
    --fps 30 \
...
```bash
python lecoro/scripts/control_sim_robot.py replay \
    --robot-path lecoro/configs/robot/your_robot_config.yaml \
    --sim-config lecoro/configs/env/your_sim_config.yaml \
    --fps 30 \
    --repo-id $USER/robot_sim_test \
...
from lecoro.common.robot_devices.control_utils import (
...
from lecoro.common.utils.utils import init_hydra_config, init_logging, log_say
...
if __name__ == "__main__":
    ...
    base_parser.add_argument(
        "--robot-path",
        type=str,
        default="lecoro/configs/robot/koch.yaml",
    ...
    parser_record.add_argument(
        ...
        type=str,
        default="lecoro/test",
        help="Dataset identifier. By convention it should match '{hf_username}/{dataset_name}' (e.g. `lecoro/test`).",
    )
    parser_record.add_argument(
        "--episode-time-s",
        type=int,
