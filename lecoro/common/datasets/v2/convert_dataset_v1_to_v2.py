...
python lecoro/common/datasets/v2/convert_dataset_v1_to_v2.py \
    --repo-id lecoro/aloha_sim_insertion_human_image \
    --single-task "Insert the peg into the socket." \
    --robot-config lecoro/configs/robot/aloha.yaml \
    --local-dir data
```
...
python lecoro/common/datasets/v2/convert_dataset_v1_to_v2.py \
    ...
    --robot-config lecoro/configs/robot/koch.yaml \
    --local-dir data
```
...
- If your dataset already contains a language instruction column in its parquet file, you can simply provide
  this column's name with the '--tasks-col' arg.
    ...
    ```bash
    python lecoro/common/datasets/v2/convert_dataset_v1_to_v2.py \
        --repo-id lecoro/stanford_kuka_multimodal_dataset \
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Repository identifier on Hugging Face: a community or a user name `/` the name of the dataset (e.g. `lecoro/pusht`, `cadene/aloha_sim_insertion_human`).",
    )
