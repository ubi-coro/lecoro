...
"""
Use this script to batch encode lecoro dataset from their raw format to LeRobotDataset and push their updated
...
lecoro-raw were downloaded in 'raw/datasets/directory':
```bash
python lecoro/common/datasets/push_dataset_to_hub/_encode_datasets.py \
  --raw-dir raw/datasets/directory \
  --raw-repo-ids lecoro-raw \
...
from lecoro.common.datasets.push_dataset_to_hub._download_raw import AVAILABLE_RAW_REPO_IDS
...
def encode_datasets(
    ...
    if len(raw_repo_ids) == 1 and raw_repo_ids[0].lower() == "lecoro-raw":
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["lecoro-raw"],
        help="""Raw dataset repo ids. if 'lecoro-raw', the keys from `AVAILABLE_RAW_REPO_IDS` will be
            used and raw datasets will be fetched from the 'lecoro-raw/' repo and pushed with their
    ...
    parser.add_argument(
        ...
        (e.g. `data/lecoro/aloha_mobile_chair`).""",
    )
    parser.add_argument(
        "--push-repo",
        type=str,
