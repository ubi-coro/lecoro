...
class BackwardCompatibilityError(Exception):
    def __init__(self, repo_id, version):
        message = textwrap.dedent(f"""
            ...
            ```
            python lecoro/common/datasets/v2/convert_dataset_v1_to_v2.py \\
                --repo-id {repo_id} \\
...
def create_lerobot_dataset_card(
    ...
    **kwargs,
    ...
    """
    Keyword arguments will be used to replace values in ./lecoro/common/datasets/card_template.md.
    ...
    card_template = (importlib.resources.files("lecoro.common.datasets") / "card_template.md").read_text()
...
def add_modality_prefix(key: str) -> str:
    """
    Prepends the modality prefix to a given key, e.g., turning 'cam_low' to 'observation.rgb.cam_low'.

    Args:
        key (str): The observation key to modify.
...
def remove_modality_prefix(key: str, return_modality: bool = False) -> str:
    """
    Strips the modality prefix from a given key, e.g., turning 'observation.rgb.cam_low' to 'cam_low'.
