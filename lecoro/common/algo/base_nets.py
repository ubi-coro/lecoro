...
class ShallowConv(ConvBase):
    ...
    """
    def __init__(self, input_channel=3, num_channel=32):
        super(ShallowConv, self).__init__()
        self._input_channel = input_channel
        self._output_channel = num_channel
        self.nets = nn.Sequential(
            torch.nn.Conv2d(input_channel, num_channel, kernel_size=7, stride=2, padding=3),
            torch.nn.ReLU(),
            torch.nn.Conv2d(num_channel, num_channel, kernel_size=1, stride=1, padding=0),
            torch.nn.ReLU(),
            torch.nn.Conv2d(num_channel, num_channel, kernel_size=3, stride=1, padding=1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(num_channel, num_channel, kernel_size=3, stride=1, padding=1),
            torch.nn.ReLU()
        )
